...
| `?internal=1`<br>`?internal=0` | Marks this browser as the owner's (a `gm_internal` cookie, one year). Every lead it files after that, partial or finished, lands with Status **Test** instead of Partial/New, so the inbound rep and the reports skip it. Emails and analytics still go as normal, so the test still exercises them. `?internal=0` removes the mark. Inside the groundmounts.com iframe that cookie is third-party and the browser drops it, so `?internal=1` on the app's own URL also counts for that page load on its own: every save it makes carries `internal: true` in the body. groundmounts.com keeps its own mark and adds `internal=1` to the iframe for you. Not a secret: the worst anyone else can do is file their own lead as a test. | `/quote?internal=1` |
