...
export async function searchAddress(query: string): Promise<GeocodingFeature[]> {
  ...
  const country = process.env.NEXT_PUBLIC_COUNTRY_CODE || 'us';
  const endpoint = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
    query
  )}.json?access_token=${process.env.NEXT_PUBLIC_MAPBOX_TOKEN}&types=address,postcode&limit=5&country=${country}`;
